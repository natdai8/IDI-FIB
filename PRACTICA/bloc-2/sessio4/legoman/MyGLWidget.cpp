...
void MyGLWidget::creaBuffers ()
{
  ...
  capsa_contenidora_model_legoman();
  ...
  glBufferData(GL_ARRAY_BUFFER, sizeof(GLfloat) * m2.faces().size() * 3 * 3,
               m2.VBO_vertices(), GL_STATIC_DRAW);
...
void MyGLWidget::capsa_contenidora_model_legoman()
{
    xmin2 = xmax2 = m2.vertices()[0];
    ymin2 = ymax2 = m2.vertices()[1];
    zmin2 = zmax2 = m2.vertices()[2];

    for (unsigned int i = 3; i < m2.vertices().size(); i += 3) {
        if (m2.vertices()[i+0] > xmax2) xmax2 = m2.vertices()[i];
        if (m2.vertices()[i+0] < xmin2) xmin2 = m2.vertices()[i];
    ...
    centre_model2 = glm::vec3((xmin2+xmax2)/2.0f, ymin2, (zmin2+zmax2)/2.0f); // pies

    alturaM2 = 1.0f / (ymax2 - ymin2);
}
...
void MyGLWidget::modelTransformLegoman()
{
    glm::mat4 TG(1.0);
    TG = glm::rotate(TG, glm::radians(90.f), glm::vec3(0.0, 1.0, 0.0));
    TG = glm::scale(TG, glm::vec3(alturaM2));
    TG = glm::translate(TG, -centre_model2);
    glUniformMatrix4fv(transLoc, 1, GL_FALSE, &TG[0][0]);
}
